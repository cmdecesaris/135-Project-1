...
summary(c, loadings = TRUE, cutoff = 0.15)
```
...
Based on the loadings also seen above, we can see that when the physical features of the baby are all positive, as seen in the first four features of Comp. 1, 
```{r}
l[,1][1:4]
```
...
```{r}
c(l[,11]['Birthweight'], l[, 11]['smoker'])
```
